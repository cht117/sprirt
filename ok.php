 ...
 * OK 框架基本库所在路径
 */
define('OK_DIR', dirname(__FILE__));
 ...
 */
if(!defined('CURRENT_TIMESTAMP')) define('CURRENT_TIMESTAMP', time());

global $G_CLASS_FILES , $G_APPCLASS_FILES;

if (empty($G_CLASS_FILES))
{
    require OK_DIR . '/config/oko_class_files.php';
}
if (empty($G_APPCLASS_FILES))
{
...
//! web 核心类搜索路径
OK::import(OK_DIR.DS.'web');


//! 异常类
//OK::import(OK_DIR.DS.'exception');


/**
 * 类 OK 是 OKo 框架的核心类，提供了框架运行所需的基本服务
 *
 * 类 OK 提供 OKo 框架的基本服务，包括：
 *
 * -  设置的读取和修改；
 * -  类定义文件的搜索和载入；
 * -  对象的单子模式实现，以及对象注册和检索；
...
class OK
{
	 ...
	 * echo OK::ini('option_group/my_option');
	 * @endcode
	 *
	 ...
	 *
	 * @code php
	 * OK::loadClass('Table_Posts', array('/www/mysite/app', '/www/mysite/lib'));
	 * @endcode
	 *
	...
	static function loadClass($class_name, $dirs = null, $throw = true)
	{
		...
		{
			//TODO:错误
			require OK_DIR . DS . $G_CLASS_FILES[$class_name_l];
			return $class_name_l;
		}elseif (isset($G_APPCLASS_FILES[$class_name_l])) {
			require $G_APPCLASS_FILES[$class_name_l];
			return $class_name_l;
		}
	 ...
	 * 添加一个类搜索路径
	 *
	 * 如果要使用 OK::loadClass() 载入非 oko 的类，需要通过 OK::import() 添加类类搜索路径。
	 *
	 * 要注意，OK::import() 添加的路径和类名称有关系。
	 *
	 * 例如类的名称为 Vendor_Smarty_Adapter，那么该类的定义文件存储结构就是 vendor/smarty/adapter.php。
	 * 因此在用 OK::import() 添加 Vendor_Smarty_Adapter 类的搜索路径时，
	 * 只能添加 vendor/smarty/adapter.php 的父目录。
	 *
	 * @code php
	 * OK::import('/www/app');
	 * OK::loadClass('Vendor_Smarty_Adapter');
	 * // 实际载入的文件是 /www/app/vendor/smarty/adapter.php
	 * @endcode
	 *
	 * 由于 oko 的规范是文件名全小写，因此要载入文件名存在大小写区分的第三方库时，
	 * 应该指定 import() 方法的第二个参数。
	 *
	 * @code php
	 * OK::import('/www/app/vendor');
	 * OK::loadClass('Zend_Mail');
	 ...
	 *
	 * @code php
	 * OK::loadClassFile('Smarty.class.php', $dirs, 'Smarty');
	 * @endcode
	 *
	...
	static function loadClassFile($filename, $dirs, $class_name, $dirname = '', $throw = true)
	{
		...
		if (!class_exists($class_name, false) && ! interface_exists($class_name, false))
		{
			if ($throw)
			{
				throw new OK_ClassNotDefinedException($class_name, $path);
			}
			return false;
		}
		return $class_name;
	 ...
	 *
	 * 与 PHP 内置的 require 和 include 相比，OK::loadFile() 会多处下列特征：
	 *
	 * <ul>
	...
	static function loadFile($filename, $dirs = null, $throw = true)
	{
		...
		{
			throw new OK_IllegalFilenameException($filename);
		}
		...
		if ($throw) throw new OK_FileNotFoundException($filename);
		return false;
	}
	 ...
	 * </ul>
	 *
	 * 使用 OK::singleton() 的好处在于多次使用同一个对象时不需要反复构造对象。
	 *
	 * @code php
	 * // 在位置 A 处使用对象 My_Object
	 * $obj = OK::singleton('My_Object');
	 * ...
	 * ...
	 * // 在位置 B 处使用对象 My_Object
	 * $obj2 = OK::singleton('My_Object');
	 ...
	 * 以特定名字在对象注册表中登记一个对象
	 *
	 * 开发者可以将一个对象登记到对象注册表中，以便在应用程序其他位置使用 OK::registry() 来查询该对象。
	 * 登记时，如果没有为对象指定一个名字，则以对象的类名称作为登记名。
	 *
	 * @code php
	 * // 注册一个对象
	 * OK::register(new MyObject());
	 * .....
	 * // 稍后取出对象
	 * $obj = OK::regitry('MyObject');
	 * @endcode
	 *
	 * 当 $persistent 参数为 true 时，对象将被放入持久存储区。
	 * 在下一次执行脚本时，可以通过 OK::registry() 取出放入持久存储区的对象，并且无需重新构造对象。
	 *
	 ...
	 *
	 * @code php
	 * if (!OK::isRegistered('MyObject'))
	 * {
	 *	  OK::registry(new MyObject(), 'MyObject', true);
	 * }
	 * $app = OK::registry('MyObject');
	 * @endcode
	 *
	...
	{
		//return OK::register($obj, $name, $persistent);
		if (!is_object($obj))
		{
			// LC_MSG: Type mismatch. $obj expected is object, actual is "%s".
			throw new OK_Exception(__('Type mismatch. $obj expected is object, actual is "%s".',
									gettype($obj)));
		}
	...
	static function registry($name)
	{
		...
		throw new OK_Exception(__('No object is registered of name "%s".', $name));
	}
	...
	{
		//return OK::isRegistered($name);
		//if ($name == 'dbo_default') throw new OK_Exception(__('No object is registered of name "%s".', $name));
		$name = strtolower($name);
		return isset(self::$_objects[$name]);
	}
	/**
	 * 注销已注册对象
	 * @param string $name	要注销对象的名字
	 */
     ...
     * @code php
     * // 修改一个设置项
     * OK::changeIni('option_group/my_option2', 'new value');
     * @endcode
     *
     ...
     * @endcode
     *
     * 上述代码展示了 OK::changeIni() 的一个重要特性：保持已有设置的层次结构。
     *
     * 因此如果要完全替换某个设置项和其子项目，应该使用 OK::replaceIni() 方法。
     *
     ...
     * 替换已有的设置值
     *
     * OK::replaceIni() 表面上看和 OK::changeIni() 类似。
     * 但是 OK::replaceIni() 不会保持已有设置的层次结构，
     * 而是直接替换到指定的设置项及其子项目。
     *
     * @code php
     * // 假设已有的设置为
     ...
     * @endcode
     *
     * 从上述代码的执行结果可以看出 OK::replaceIni() 和 OK::changeIni() 的重要区别。
     *
     * 不过由于 OK::replaceIni() 速度比 OK::changeIni() 快很多，
     * 因此应该尽量使用 OK::replaceIni() 来代替 OK::changeIni()。
     *
     * @param string|array $option 要修改的设置项名称，或包含多个设置项目的数组
     * @param mixed $data 指定设置项的新值
     */
     ...
     *
     * OK::cache() 可以指定 $policy 参数来覆盖缓存数据本身带有的策略。
     * 具体哪些策略可以使用，请参考不同缓存服务的文档。
     *
     * $backend_class 用于指定要使用的缓存服务对象类名称。例如 OK_Cache_File、OK_Cache_APC 等。
     *
     * @code php
     * $data = OK::cache($cache_id);
     * if ($data === false)
     * {
     *     $data = ....
     *     OK::writeCache($cache_id, $data);
     * }
     * @endcode
     *
    ...
    static function cache($id, array $policy = null, $backend_class = null)
    {
        ...
        if (is_null($backend_class))
        {
            if (is_null($obj))
            {
                $obj = OK::singleton(self::ini('runtime_cache_backend'));
            }
            return $obj->get($id, $policy);
        }
        else
        {
            $cache = OK::singleton($backend_class);
    ...
    static function writeCache($id, $data, array $policy = null, $backend_class = null)
    {
        ...
        {
        	/*  @var $cache OK_Cache_PHPDataFile */
            $cache = OK::singleton($backend_class);
	 ...
	 */
	static function registerAutoload ($class = 'OK', $enabled = true)
	{
		if (! function_exists('spl_autoload_register')) {
			throw new OK_Exception('spl_autoload does not exist in this PHP installation');
		}
		if ($enabled === true) {
     ...
     * 创建一个用户界面控件对象
     *
     * 使用 OK::control() 方法，可以很容易的创建指定类型的用户界面控件对象。
     *
     ...
     *
     *
     * @return OK_UI_Control_Abstract 创建的用户界面控件对象
     */
 ...
 * @param bool $exit
 */
function pecho($data = null, $exit = true)
{
	echo "<pre>";
	print_r($data);
	echo "</pre>";
 ...
 * OKo 内部使用的多语言翻译函数
 *
 * 应用程序应该使用 OK_Translate 组件实现多语言界面。
 *
 * @return $msg
 */
function __()
{
	...
	$language = strtolower(OK::ini('error_language'));
	$messages = OK::loadFile('lc_messages.php', OK_DIR . '/_lang/' . $language, false);
...
{
	pecho(array($vars, $label, $return) , false);
    //return OK_Debug::dump($vars, $label, $return);
}
...
{
	if (ok::ini('new_url_function')) {
		return OK_Context::instance()->url($udi, $params, $route_name, $opts);
	}
	...
	$udi = array(
		'action'		=> array_pop($udi),
		'controller'	=> array_pop($udi),
		'module'		=> array_pop($udi),
	);
	if (empty($udi['module'])) unset($udi['module']);
	$udi = array_reverse($udi);
	
	$url .= '?' . http_build_query($udi, '', '&');
	
	if (! empty($params)) {
		$url .= '&' . http_build_query($params, '', '&');
	}
	if (! is_null($route_name) && ! is_null($opts)) {
		pecho(array($udi, $params, $route_name, $opts));
	}
	//pecho($url);
	return $url;
